...
namespace RedShirt.Example.JobWorker.JobManagement.RabbitMq.Subscribe.Services.Resilience;

/// <summary>
///     Retries RabbitMQ client operations that fail with expected transient exceptions,
...
/// </summary>
internal interface IRabbitMqRetryWrapperService
{
    /// <summary>
...
/// <summary>
///     Polly v8-based retry wrapper for RabbitMQ client calls.
///     Retries when <see cref="IRabbitMqExceptionArbiterService" /> reports an expected transient failure,
///     using exponential backoff via <see cref="ISleepService" />.
/// </summary>
/// <param name="exceptionArbiterService">Classifies RabbitMQ-related exceptions as expected/transient.</param>
...
internal class RabbitMqRetryWrapperService(
    IRabbitMqExceptionArbiterService exceptionArbiterService,
    ILogger<RabbitMqRetryWrapperService> logger,
    ISleepService sleepService)
    : IRabbitMqRetryWrapperService
{
    private const int RabbitMqRetryCount = 3;
    ...
    private ResiliencePipeline GetRetryPipeline()
    {
        return _retryPipeline ??= new ResiliencePipelineBuilder()
            ...
            {
                MaxRetryAttempts = RabbitMqRetryCount,
                ShouldHandle = args =>
                {
                ...
                OnRetry = async args =>
                {
                    logger.LogWarning(args.Outcome.Exception,
                        "Retrying RabbitMQ operation after attempt {AttemptNumber}",
                        args.AttemptNumber);
