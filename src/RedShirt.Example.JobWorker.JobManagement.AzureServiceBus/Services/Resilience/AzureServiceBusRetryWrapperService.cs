...
/// <summary>
///     Retries Azure Service Bus job-source operations that fail with expected transient exceptions,
...
/// </summary>
internal interface IAzureServiceBusRetryWrapperService
{
    /// <summary>
    ///     Executes <paramref name="func" /> with retry for expected transient Service Bus failures.
    /// </summary>
...
/// <summary>
///     Polly v8-based retry wrapper for Azure Service Bus client calls.
///     Retries when <see cref="IAzureServiceBusExceptionArbiterService" /> reports an expected transient failure,
///     using exponential backoff via <see cref="ISleepService" />.
/// </summary>
/// <param name="exceptionArbiterService">Classifies Service Bus-related exceptions as expected/transient.</param>
...
internal class AzureServiceBusRetryWrapperService(
    IAzureServiceBusExceptionArbiterService exceptionArbiterService,
    ILogger<AzureServiceBusRetryWrapperService> logger,
    ISleepService sleepService)
    : IAzureServiceBusRetryWrapperService
{
    private const int AzureServiceBusRetryCount = 3;
    ...
    private ResiliencePipeline GetRetryPipeline()
    {
        return _retryPipeline ??= new ResiliencePipelineBuilder()
            ...
            {
                MaxRetryAttempts = AzureServiceBusRetryCount,
                ShouldHandle = args =>
                {
                ...
                OnRetry = async args =>
                {
                    logger.LogWarning(args.Outcome.Exception,
                        "Retrying Azure Service Bus operation after attempt {AttemptNumber}",
                        args.AttemptNumber);
