...
/// <summary>
///     Retries Google Pub/Sub client operations that fail with expected transient exceptions,
...
{
    /// <summary>
    ///     Executes <paramref name="func" /> with retry for expected transient Pub/Sub failures.
    /// </summary>
...
///     Retries when <see cref="IGooglePubSubExceptionArbiterService" /> reports an expected transient failure,
///     using exponential backoff via <see cref="ISleepService" />.
/// </summary>
/// <param name="exceptionArbiterService">Classifies Pub/Sub-related exceptions as expected/transient.</param>
...
internal class GooglePubSubRetryWrapperService(
    ...
    ILogger<GooglePubSubRetryWrapperService> logger,
            ...
            .AddRetry(new RetryStrategyOptions
            {
                ...
                OnRetry = async args =>
                {
                    logger.LogWarning(args.Outcome.Exception,
                        "Retrying Google Pub/Sub operation after attempt {AttemptNumber}",
                        args.AttemptNumber);
