...
/// <summary>
///     Retries Azure Queue Storage job-source operations that fail with expected transient exceptions,
...
/// </summary>
internal interface IAzureQueueStorageRetryWrapperService
{
    /// <summary>
    ///     Executes <paramref name="func" /> with retry for expected transient Queue Storage failures.
    /// </summary>
...
/// <summary>
///     Polly v8-based retry wrapper for Azure Queue Storage client calls.
///     Retries when <see cref="IAzureQueueStorageExceptionArbiterService" /> reports an expected transient failure,
///     using exponential backoff via <see cref="ISleepService" />.
/// </summary>
/// <param name="exceptionArbiterService">Classifies Queue Storage-related exceptions as expected/transient.</param>
...
internal class AzureQueueStorageRetryWrapperService(
    IAzureQueueStorageExceptionArbiterService exceptionArbiterService,
    ILogger<AzureQueueStorageRetryWrapperService> logger,
    ISleepService sleepService)
    : IAzureQueueStorageRetryWrapperService
{
    private const int AzureQueueStorageRetryCount = 3;
    ...
    private ResiliencePipeline GetRetryPipeline()
    {
        return _retryPipeline ??= new ResiliencePipelineBuilder()
            ...
            {
                MaxRetryAttempts = AzureQueueStorageRetryCount,
                ShouldHandle = args =>
                {
                ...
                OnRetry = async args =>
                {
                    logger.LogWarning(args.Outcome.Exception,
                        "Retrying Azure Queue Storage operation after attempt {AttemptNumber}",
                        args.AttemptNumber);
