...
/// <summary>
///     Retries ActiveMQ client operations that fail with expected transient exceptions,
...
/// </summary>
internal interface IActiveMqRetryWrapperService
{
    /// <summary>
    ///     Executes <paramref name="func" /> with retry for expected transient ActiveMQ failures.
    /// </summary>
...
/// <summary>
///     Polly v8-based retry wrapper for ActiveMQ client calls.
///     Retries when <see cref="IActiveMqExceptionArbiterService" /> reports an expected transient failure,
///     using exponential backoff via <see cref="ISleepService" />.
/// </summary>
/// <param name="exceptionArbiterService">Classifies ActiveMQ-related exceptions as expected/transient.</param>
...
internal class ActiveMqRetryWrapperService(
    IActiveMqExceptionArbiterService exceptionArbiterService,
    ILogger<ActiveMqRetryWrapperService> logger,
    ISleepService sleepService)
    : IActiveMqRetryWrapperService
{
    private const int ActiveMqRetryCount = 3;
    ...
    private ResiliencePipeline GetRetryPipeline()
    {
        return _retryPipeline ??= new ResiliencePipelineBuilder()
            ...
            {
                MaxRetryAttempts = ActiveMqRetryCount,
                ShouldHandle = args =>
                {
                ...
                OnRetry = async args =>
                {
                    logger.LogWarning(args.Outcome.Exception,
                        "Retrying ActiveMQ operation after attempt {AttemptNumber}",
                        args.AttemptNumber);
