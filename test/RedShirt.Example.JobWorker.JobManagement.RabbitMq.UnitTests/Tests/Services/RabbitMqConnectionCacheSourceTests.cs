...
public class RabbitMqConnectionCacheSourceTests
{
    ...
    public async Task GetConnectionAsync_FirstCall_CreatesUncachedConnection()
    {
        ...
        var result = await source.GetConnectionAsync(false, TestContext.Current.CancellationToken);
    ...
    public async Task GetConnectionAsync_SecondCallWithoutForce_ReturnsCachedConnection()
    {
        ...
        var first = await source.GetConnectionAsync(false, TestContext.Current.CancellationToken);
        var second = await source.GetConnectionAsync(false, TestContext.Current.CancellationToken);
    ...
    public async Task GetConnectionAsync_WhenForceNewConnection_CreatesReplacement()
    {
        ...
        var first = await source.GetConnectionAsync(false, TestContext.Current.CancellationToken);
        var forced = await source.GetConnectionAsync(true, TestContext.Current.CancellationToken);
