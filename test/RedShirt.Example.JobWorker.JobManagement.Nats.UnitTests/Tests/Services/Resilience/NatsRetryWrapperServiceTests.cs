...
public class NatsRetryWrapperServiceTests
{
    private static NatsExceptionArbiterReport TransientReport()
    {
    ...
    private static NatsExceptionArbiterReport NonTransientReport()
    {
        return new NatsExceptionArbiterReport
        {
            AlreadyHandled = false,
    ...
    private static NatsExceptionArbiterReport AlreadyHandledReport(bool couldBeTransient)
    {
        return new NatsExceptionArbiterReport
        {
            AlreadyHandled = true,
    ...
    private static NatsExceptionArbiterReport CriticalTransientReport()
    {
        return new NatsExceptionArbiterReport
        {
            AlreadyHandled = false,
    ...
    private static NatsExceptionArbiterReport UnexpectedReport()
    {
        return new NatsExceptionArbiterReport
        {
            AlreadyHandled = false,
    ...
    public async Task RunAsync_NonGeneric_WhenAlreadyHandled_RethrowsWithoutWrapping()
    {
        ...
        var arbiter = new Mock<INatsExceptionArbiterService>(MockBehavior.Strict);
        arbiter.Setup(a => a.GetReport(inner)).Returns(AlreadyHandledReport(false));

        var sleepService = CreateSleepService();
        var wrapper = new NatsRetryWrapperService(arbiter.Object, NullLogger<NatsRetryWrapperService>.Instance,
            sleepService.Object);
    ...
    public async Task RunAsync_WhenAlreadyHandledCritical_RethrowsSameInstanceWithoutRetry()
    {
        ...
        var arbiter = new Mock<INatsExceptionArbiterService>(MockBehavior.Strict);
        arbiter.Setup(a => a.GetReport(inner)).Returns(new NatsExceptionArbiterReport
        {
            AlreadyHandled = true,
            IsExpected = false,
            CouldBeTransient = false,
