...
public class LoaderModeJobLoaderTests
{
    ...
    public async Task RunAsync_WhenBacklogHasCapacity_FetchesMinOfFreeSlotsAndBatchSizeThenSubmits()
    {
        ...
        jobLoaderLoop
            .Setup(l => l.RunAsync(It.IsAny<Func<CancellationToken, Task>>(), TestContext.Current.CancellationToken))
            .Returns(async (Func<CancellationToken, Task> iteration, CancellationToken ct) =>
            {
                await iteration(ct);
        ...
        jobLoaderLoop.Verify(
            l => l.RunAsync(It.IsAny<Func<CancellationToken, Task>>(), TestContext.Current.CancellationToken),
            Times.Once);
    }
